...
// 如果有右子树，下个节点是右子树的最左节点
// 如果没有右子树，且是父节点的左子树，下个节点是父节点，
//            如果是父节点的右子树，向上回溯直到一个节点是父节点左子树，下个节点就是此节点的父节点
...
void Test(char* testName, BinaryTreeNode* pNode, BinaryTreeNode* expected) {
    ...
    if (pNext == expected)
        ...
        printf("FAILED. pNext is %i, expected %i\n",
               pNext == nullptr ? 0 : pNext->m_nValue,
               expected == nullptr ? 0 : expected->m_nValue);
}
