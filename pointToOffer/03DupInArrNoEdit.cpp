...
// 与二分查找相似,对于长度为n的数组,countRange
// 将被调用O(logn)次,每次遍历O(n)时间,时间复杂度O(nlogn), 空间复杂度O(1),
// 相当于以时间换空间 注, 只能找出至少一个,不能找出所有重复数字
...
void test10() {
    int* numbers = nullptr;
    int duplications[] = {-1};
    test("test10", numbers, 0, duplications,
         sizeof(duplications) / sizeof(int));
}
void test11() {
    int numbers[] = {2, 3, 5, 4, 3, 2, 6, 7};
    int duplications[] = {2, 3};
    test("test11", numbers, sizeof(numbers) / sizeof(int), duplications,
         sizeof(duplications) / sizeof(int));
}
